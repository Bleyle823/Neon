...
import { z } from 'zod';
import { WhatsAppPlugin as WhatsAppClientPlugin } from '@elizaos/plugin-whatsapp';
...
// Initialize WhatsApp client (if env present)
const whatsapp =
  ...
  routes: [
    ...
    // WhatsApp webhook receiver (POST)
    {
      ...
      handler: async (req: any, res: any) => {
        ...
        try {
          // Let the client do any built-in handling/logging
          ...
          for (const m of messages) {
            ...
            const textBody =
              m?.text?.body ??
              (m?.type === 'text' ? m?.text?.body : undefined);
            if (from && textBody) {
              const reply =
                `Neon received: "${textBody}".` +
                ` I’m online and will get back shortly.`;
